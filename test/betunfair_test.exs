defmodule BetUnfairTest do
  ...
  test "match_bets1" do
    ...
    assert [^bl1,^bl2] = Enum.to_list(lays) |> Enum.map(fn (e) -> elem(e,1) end)
    assert is_ok(BetUnfair.market_match(m1))
    assert {:ok,%{remaining_stake: 500}} = BetUnfair.bet_get(bb1)
  ...
  test "match_bets3" do
    ...
    assert is_ok(BetUnfair.market_match(m1))
    assert {:ok,%{remaining_stake: 900}} = BetUnfair.bet_get(bb1)
    assert {:ok,%{remaining_stake: 0}} = BetUnfair.bet_get(bl2)
  ...
  test "match_bets5" do
    ...
    assert {:ok,%{balance: 2050}} = BetUnfair.user_get(u1)
  ...
  test "match_bets6" do
    ...
    assert {:ok,%{balance: 1900}} = BetUnfair.user_get(u1)
    assert {:ok,%{balance: 2100}} = BetUnfair.user_get(u2)
    assert :ok = BetUnfair.clean("testdb")
  end
